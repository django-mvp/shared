...
Downstream repos previously mixed two hook styles: mypy/deptry as `repo: local` hooks
running in the Poetry env, but ruff/black/pyupgrade as upstream pre-commit mirrors with
per-repo `rev:` pins bumped by autoupdate. Consequences: tool versions drifted per repo
outside the shared bundle's control, and `poetry run ruff` didn't work locally because
ruff wasn't in the env (each repo's verify tooling had to special-case it).

## Decision

The shared pre-commit template (`templates/pre-commit-config.yaml`) runs **all Python
...
- pre-commit.ci can only run the env-independent hooks; the local hooks run in each repo's
  CI Code Quality job (already the pattern in every downstream repo).
