# ADR 0002 — One repo carries the whole shared standard

**Status:** Accepted (2026-07-21)

## Context

The shared standard has three parts: reusable CI workflows, the shared toolchain (the
...
framework (fairdm) should consume the generic shared standard, not define it.
...
replaces `fairdm-dev-tools` as the toolchain source for every repository that adopts this
standard.
...
- Correct dependency direction: generic shared standard at the bottom; domain frameworks
...
- `fairdm-dev-tools` becomes a consumer-side concern of fairdm and its own dependants, and
  can be retired or reduced to fairdm-specific extras.
